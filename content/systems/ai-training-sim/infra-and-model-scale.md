...
算力不应只是一种统一货币。训练、推理、不同精度、不同互联和不同供电条件会给玩家提供不同约束。

高精度训练算力影响前沿模型训练稳定性、checkpoint 质量和能力上限。低精度推理算力影响部署成本、API 毛利、普及速度和降智压力。电力价格、供电稳定性和是否能购买海外算力，会继续影响模型强度、服务规模和公司能否按时发布。

这些变量先作为基础设施接口存在，不在本页写成完整数值公式。
